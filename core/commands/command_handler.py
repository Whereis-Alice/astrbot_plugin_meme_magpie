...
class CommandHandler:
    ...
    async def migrate_legacy(
        ...
        if service is None:
            yield event.plain_result("❌ 迁移服务不可用，请检查插件是否完整安装")
            return
        ...
        if action == "check":
            info = service.detect_legacy(source or None)
            if not info.get("found"):
                yield event.plain_result(f"未检测到旧插件数据。\n{info.get('reason', '')}")
                return
        ...
        apply = action in {"apply", "move"}
        move = action == "move"
        if apply:
            yield event.plain_result("开始迁移，数据量大时可能需要几分钟，请稍候…")
        try:
            report = await service.migrate(source=source or None, apply=apply, move=move)
