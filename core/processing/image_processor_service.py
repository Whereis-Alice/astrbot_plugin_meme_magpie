...
class ImageProcessorService:
    ...
    async def _store_and_index_image(
        ...
        if extra_meta and isinstance(extra_meta, dict):
            # Avoid overriding core fields the plugin relies on.
    ...
    async def process_image(
        ...
        Args:
            ...
            llm_hints: 主对话 LLM 调用偷图工具时自带的结构化标注（作品/角色/动作……）。
                不为 None 时优先级高于 VLM 结果，但不能绕过内容审核。
            skip_vlm: 跳过视觉模型调用，完全用 ``llm_hints`` 入库（省 token）。
                仍会走哈希/感知哈希查重、raw 落盘、索引与向量写入。
    ...
    async def _handle_classification_result(
        ...
        ``llm_hints`` 不为 None 时，LLM 显式传入的字段优先于 VLM 结果，
        VLM 只负责补齐 LLM 没传的部分。但内容审核结论（过滤不通过 /
        非表情包）不可被 LLM 参数绕过。
        """
        ...
        # LLM 主动偷图时自带的标注优先生效（需求 2）。
        # 放在无效分类兜底之前：LLM 给了合法分类就不应该再被兜底到 confused。
        if llm_hints is not None and llm_hints.has_semantics:
            if llm_hints.category:
                if category and category != llm_hints.category:
                    logger.debug(
                        f"分类以 LLM 传参为准: {category!r} -> {llm_hints.category!r}"
                    )
                category = llm_hints.category
            tags = llm_hints.merge_tags(tags)
            scenes = llm_hints.merge_scenes(scenes)
            emotions = llm_hints.merge_emotions(emotions)
            desc = llm_hints.resolve_desc(desc)
            overlay_text = llm_hints.resolve_overlay_text(overlay_text)
        ...
        if llm_hints is not None:
            # character / work / overlay_text / emotions 直接合入 entry，
            # 下游 _store_and_index_image / _store_to_pending 会透传到 DB。
            semantic_meta.update(llm_hints.to_extra_meta())
