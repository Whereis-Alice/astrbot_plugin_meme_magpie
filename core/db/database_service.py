...
class DatabaseService:
    ...
    def __init__(self, db_path: str | Path | None = None):
        ...
        if db_path is None:
            ...
            db_path = Path(StarTools.get_data_dir("astrbot_plugin_meme_magpie")) / "emoji.db"
    ...
    def _create_tables(self, conn: sqlite3.Connection) -> None:
        ...
        try:
            ...
            pass
        try:
            conn.execute("CREATE INDEX IF NOT EXISTS idx_emoji_work ON emoji(work)")
        except sqlite3.OperationalError:
            pass
    ...
    # v7 新增：作品名（番剧/游戏/漫画名）。单独一组，不往 _SEMANTIC_COLUMNS 里塞，
    # 否则已升到 v6 的旧库不会再跑 _migrate_v6，work 列就永远不会被创建。
    ...
    def _migrate_v7(self, conn: sqlite3.Connection) -> None:
        """v6 -> v7：为 emoji / emoji_pending 添加 work（作品名）列。"""
        for col, ddl in self._WORK_COLUMNS:
            for table in ("emoji", "emoji_pending"):
                try:
                    conn.execute(f"ALTER TABLE {table} ADD COLUMN {col} {ddl}")
                except sqlite3.OperationalError as e:
        ...
        try:
            ...
            pass
        logger.info("[DB] 迁移完成 (v7): work（作品名）就绪")
    ...
    def _hydrate_entry(self, entry: dict[str, Any]) -> dict[str, Any]:
        ...
        entry["work"] = str(entry.get("work") or "").strip()
        return entry
    ...
    def get_work_counts(self) -> dict[str, int]:
        """统计每个作品名下的表情包数量（未标注的归入空字符串）。"""
        with self._get_connection() as conn:
            rows = conn.execute(
                """
                SELECT COALESCE(work, '') as work, COUNT(*) as cnt
                FROM emoji GROUP BY COALESCE(work, '')
                """
            ).fetchall()
            return {str(r["work"] or ""): int(r["cnt"]) for r in rows}

    def clear_work(self, work: str) -> int:
        """清空指定作品名的标注（不删图），返回影响行数。"""
        key = str(work or "").strip()
        if not key:
            return 0
        with self._get_connection() as conn:
            cur = conn.execute("UPDATE emoji SET work = '' WHERE work = ?", (key,))
            conn.execute("UPDATE emoji_pending SET work = '' WHERE work = ?", (key,))
    ...
    def get_all_hashes(self) -> set[str]:
        """获取正式表中所有非空图片哈希，用于批量查重（如数据迁移）。"""
        with self._get_connection() as conn:
            rows = conn.execute(
                "SELECT hash FROM emoji WHERE hash IS NOT NULL AND hash != ''"
            ).fetchall()
            return {r["hash"] for r in rows}

    def get_all_pending_hashes(self) -> set[str]:
        """获取待审核池中所有非空图片哈希，用于批量查重（如数据迁移）。"""
        with self._get_connection() as conn:
            rows = conn.execute(
                "SELECT hash FROM emoji_pending WHERE hash IS NOT NULL AND hash != ''"
            ).fetchall()
            return {r["hash"] for r in rows}
    ...
    def get_emojis_paginated(
        ...
        Args:
            ...
            character: 角色过滤（"__none__" 表示仅未标注）
            work: 作品过滤（"__none__" 表示仅未标注）
        ...
        with self._get_connection() as conn:
            ...
            if work == "__none__":
                where_clauses.append("(e.work IS NULL OR e.work = '')")
            elif work:
                where_clauses.append("e.work = ?")
                params.append(work)
            ...
            if search_query:
                ...
                search_clause = (
                    ...
                    " OR e.overlay_text LIKE ? OR e.character LIKE ? OR e.work LIKE ?"
                    " OR EXISTS("
                ...
                category_count_params.extend([search_pattern] * 11)
    ...
    def get_pending_paginated(
        ...
        if search_query:
            ...
            where_clauses.append(
                ...
                " OR p.overlay_text LIKE ? OR p.character LIKE ? OR p.work LIKE ?"
                " OR EXISTS("
