...
| **跨插件联动** | 向论坛、图床等插件提供带权限校验的结构化候选和短期资源句柄 |
...
| [跨插件联动](docs/integration.md) | 结构化搜索、资源句柄和图床调用流程 |
...
- 开启「以 GIF 格式发送」时，已是 GIF 的文件会原样透传；其他动图需要转换，峰值内存虽有帧数与总像素预算封顶，仍高于直接发原图，内存紧张的机器建议关闭。
